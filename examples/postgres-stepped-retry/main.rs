...
async fn main() -> Result<(), Error> {
    ...
    // Define retry policy
    let retry_policy = RetryPolicy::retries(10).with_backoff(
    ...
    let wf1_steps = StepBuilder::new()
        // step_1 (utilizing 'internal' 5-retry policy without backoff)
        .step_fn(wf1::step_1)
        // step_2 (with retry and backoff)
        .step(RetryLayer::new(retry_policy).layer(service_fn(wf1::step_2)));
...
mod wf1 {
    ...
    pub async fn step_2(req: Step2) -> Result<GoTo<()>, Error> {
        ...
        // Get second value of the current time.
        let sec = now.second();
        if sec <= 50 {
            println!(">>> step_2 failed");
            return Err(Error::ArtificialStepError);
        } else {
            println!(">>> step_2 succeeded")
        }
...
pub enum Error {
    ...
    #[error("Artificial step error")]
    ArtificialStepError,

    #[error("Invalid backoff: {0}")]
    InvalidBackoff(#[from] tower::retry::backoff::InvalidBackoff),
}
