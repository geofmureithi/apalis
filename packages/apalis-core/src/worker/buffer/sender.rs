...
use super::reusable::ReusableBoxFuture;
...
type InnerFuture<'a, T> = ReusableBoxFuture<'a, Result<(), PollSendError<T>>>;
...
impl<T: Send + Clone> PollSenderFuture<T> {
    ...
    fn poll(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), PollSendError<T>>> {
        self.0.poll(cx)
    }
...
impl<T: Send + Clone> PollSender<T> {
        ...
        loop {
            let (result, next_state) = match self.take_state() {
                ...
                State::Acquiring => match self.acquire.poll(cx) {
                    ...
                    Poll::Ready(Ok(())) => {
                        (Some(Poll::Ready(Ok(()))), State::ReadyToSend)
                    }
                    // Channel is closed.
                ...
                s @ State::ReadyToSend => (Some(Poll::Ready(Ok(()))), s),
            };
            ...
            State::ReadyToSend => {
                if let Some(mut sender) = self.sender.clone() {
                    match sender.try_send(value.clone()) {
                        Ok(_) => (Ok(()), State::Idle(sender)),
                        Err(_) => (Err(PollSendError(Some(value))), State::Closed),
                    }
                } else {
                    (Err(PollSendError(Some(value))), State::Closed)
                }
            }
            // We're closed, either by choice or because the underlying sender was closed.
    ...
    /// Checks whether this sender has been closed.
    ///
        ...
        let (result, next_state) = match self.take_state() {
            ...
            State::ReadyToSend => {
                let state = if self.sender.is_some() {
                    State::Idle(self.sender.clone().unwrap())
                } else {
                    State::Closed
                };
                (true, state)
...
impl<T: Send + Clone> Sink<T> for PollSender<T> {
