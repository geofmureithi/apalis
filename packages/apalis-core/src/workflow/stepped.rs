...
use crate::{
    backend::{
        ...
        Backend, BackendWithCodec, Push,
    },
    error::BoxDynError,
    request::Request,
    worker::{
        builder::{BuildWith, WorkerBuilder, WorkerFactory},
        Worker,
    },
};
...
impl<Ctx, Compact, Input, Current, Codec> Service<Request<StepRequest<Compact>, Ctx>>
    for StepBuilder<Input, Current, SteppedService<Compact, Ctx>, Codec>
{
...
impl<Ctx, Compact, Input, Current, Codec, B, M>
    BuildWith<WorkerBuilder<Request<StepRequest<Compact>, Ctx>, B, M>>
    for StepBuilder<Input, Current, SteppedService<Compact, Ctx>, Codec>
where
    B: BackendWithCodec<Request<StepRequest<Compact>, Ctx>, Codec = Codec, Compact = Compact> + Push<StepRequest<Compact>, Ctx> + Clone,
    M: Layer<SteppedService<Compact, Ctx>>,
    Input: Send + 'static,
    Current: Send + 'static,
    Codec: Send + 'static,
    Ctx: 'static,
    Compact: 'static + Send,
{
    type Worker = Worker<StepRequest<Compact>, Ctx, B, SteppedService<Compact, Ctx>, M>;
    fn builder_with(
        self,
        builder: WorkerBuilder<Request<StepRequest<Compact>, Ctx>, B, M>,
    ) -> Self::Worker {
        
        let svc = ServiceBuilder::new()
            // .map_future(f)
            // .and_then(|res| async move { match &res {
            //     GoTo::Next(next) => {
            //         backend.push(StepRequest {
            //                         index: next_index,
            //                         step: resp.clone(),
            //                     })
            //                     .await
