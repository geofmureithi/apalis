...
CREATE FUNCTION apalis.get_job(worker_id TEXT, v_job_type TEXT) returns apalis.jobs AS $ $ DECLARE v_job_id text;
...
IF v_job_id IS NULL THEN RETURN NULL;
...
SET
    ...
    lock_at = now()
WHERE
    id = v_job_id returning * INTO v_job_row;
...
CREATE FUNCTION apalis.notify_new_jobs() returns trigger as $ $ begin perform pg_notify('apalis::job', 'insert');
...
AFTER
INSERT
    ON apalis.jobs FOR EACH STATEMENT execute procedure apalis.notify_new_jobs();