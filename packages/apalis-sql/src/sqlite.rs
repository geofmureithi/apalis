    ...
    fn stream_jobs(
        ...
        try_stream! {
            loop {
                interval.tick().await;
                let tx = pool.clone();
                let mut tx = tx.acquire().await.map_err(|e| StorageError::Database(Box::from(e)))?;
                let job_type = T::NAME;
                ...
                let job: Option<SqlJobRequest<T>> = sqlx::query_as(fetch_query)
                    ...
                    .bind(job_type)
                    .fetch_optional(&mut tx)
                    .await.map_err(|e| StorageError::Database(Box::from(e)))?;
                yield fetch_next(pool.clone(), worker_id.clone(), job.as_job_request()).await.map_err(|e| StorageError::Database(Box::from(e)))?
            }
        }
    }
}
...
impl<J: 'static + Job + Serialize + DeserializeOwned> StorageJobExt<J> for SqliteStorage<J> {
    fn list_jobs(&mut self, status: &JobState, _page: i32) -> StorageResult<Vec<JobRequest<J>>> {
        let pool = self.pool.clone();
